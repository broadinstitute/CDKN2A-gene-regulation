...
## pgParams
params <- pgParams(
  chrom = "chr9", chromstart = 20300000, chromend = 22600000,
  assembly = "hg38",
  x = 1, just = c("left", "top"),
...
params2 <- pgParams(
  chrom = "9", chromstart = 20300000, chromend = 22600000,
  assembly = "hg38",
  x = 1, just = c("left", "top"),
  width = 8, length = 2, default.units = "inches"
)

pdf('plotGardener_Wi38_late_min_early_chr9_20300000_22600000_with_isoforms.pdf', width =10 , height = 8.5)
...
hicPlot<-plotHicTriangle(
  ...
  just = c("left", "top"), default.units = "inches", colorTrans='linear', zrange=c(-10,10), palette = colorRampPalette(rev(brewer.pal(n = 9, "RdBu"))))
...
## Add text labels
plotText(
  label = "MicroC PDL48 - PDL21, normalized by total reads in chr9, 5kb res", fontsize = 7, fontcolor = "darkred",
...
plotGenomeLabel(
  params= params,
  x = 1, y = 7.5, length = 8, scale = "bp", at =seq(20300000, 22600000, by=50000), 
